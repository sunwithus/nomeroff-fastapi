...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Загрузка модели nomeroff-net (RU-only)...")
    ...
    logger.info("✅ Модель загружена (default_label=ru, latin→cyrillic)")
    yield
    ...
    try:
        ...
        for plate_text in all_texts:
            ...
            if not isinstance(plate_text, str):
                continue
            plate_clean = latin_to_cyrillic(plate_text.strip())
            if plate_clean:
